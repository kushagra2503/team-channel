 ...
 * reconciliation. Keep identical until it is lifted into @teambridge/core
 ...
 * (`teambridge/<sessionName>/<safeName>`). Reject anything that would produce
...
export function worktreePathFor(repoRoot: string, sessionName: string, displayName: string): string {
  return join(repoRoot, '.teambridge', 'worktrees', sessionSlug(sessionName), safeDisplayName(displayName));
}
