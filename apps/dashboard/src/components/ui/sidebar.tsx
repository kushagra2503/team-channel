...
const SIDEBAR_EXPANDED_PX = 256
  const SIDEBAR_ICON_PX = 48
  const collapsed = state === "collapsed"
  const isOffcanvas = collapsible === "offcanvas"
  const isIcon = collapsible === "icon"
  const sideIsRight = side === "right"
  const gapWidth = collapsed ? (isOffcanvas ? 0 : isIcon ? SIDEBAR_ICON_PX : SIDEBAR_EXPANDED_PX) : SIDEBAR_EXPANDED_PX
  const containerWidth = collapsed ? (isIcon ? SIDEBAR_ICON_PX : SIDEBAR_EXPANDED_PX) : SIDEBAR_EXPANDED_PX
  const containerOffset = collapsed && isOffcanvas ? SIDEBAR_EXPANDED_PX : 0
  const sidebarSpring = { type: "spring" as const, duration: 0.3, bounce: 0 }
      ...
      <motion.div
        ...
        className={cn(
          ...
          variant === "floating" || variant === "inset"
            ? "p-2 group-data-[side=left]:border-r group-data-[side=right]:border-l"
...
const sidebarMenuButtonVariants = cva(
  "peer/menu-button group/menu-button flex w-full items-center gap-2 overflow-hidden rounded-[calc(var(--radius-sm)+2px)] p-2 text-left text-xs ring-sidebar-ring outline-hidden transition-[width,height,padding,background-color,color] duration-150 ease-out group-has-data-[sidebar=menu-action]/menu-item:pr-8 group-data-[collapsible=icon]:size-8! group-data-[collapsible=icon]:p-2! hover:bg-sidebar-accent hover:text-sidebar-accent-foreground focus-visible:ring-2 active:bg-sidebar-accent active:text-sidebar-accent-foreground disabled:pointer-events-none disabled:opacity-50 aria-disabled:pointer-events-none aria-disabled:opacity-50 data-open:hover:bg-sidebar-accent data-open:hover:text-sidebar-accent-foreground data-active:bg-sidebar-accent data-active:font-medium data-active:text-sidebar-accent-foreground [&_svg]:size-4 [&_svg]:shrink-0 [&>span:last-child]:truncate",
  {
    variants: {
      variant: {
